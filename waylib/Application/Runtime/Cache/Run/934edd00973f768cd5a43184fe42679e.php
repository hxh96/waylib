...
<html>
	...
	<body>
		...
		<?php if(($status == 1)): ?><div class="choose">
				<div class="kind">
					<form class="outer">
						<div class="choose8">
							<select id="type">
								<option value="u_mobile">手机号</option>
								<option value="u_name">读者证号</option>
							</select>
							<input type="text" class="input-medium" placeholder="请输入内容" id="content" value="<?php echo ($content); ?>"/>
						</div>
						<div class="search">
							<img src="/Public/Run/img/search.png" id="search_content"/>
						</div>
					</form>
				</div>
			</div>
			<script>
				$(function(){
					...
					$('#search_content').click(function(){
						var type = $("#type option:selected").val();//获取当前选择项的值.
						var content = $('#content').val();
						if(!content){
							return dialog.error('请输入内容');
						}else {
							location.href = "<?php echo U('App/userBack');?>?t="+type+'&ct='+content;
						}
					});
				})
			</script>
		...
		<div class="main_biaoge">
			<table  class="table1 table-bordered table-striped table table9 ">
					<tr>
						<th>姓名</th>
						<th>电话号码</th>
						<th>读者证号</th>
