	...
	<div id="LoginBox12">
		<div class="api_list">
			...
			<div class="api_name outer">
				<p>图书馆：</p><span><?php echo ($libName); ?></span>
	...
	<script>
		$(function(){
			...
			$('#addApi').click(function(){
				...
				$.ajax({
					type:'post',
					url:"<?php echo U('Library/addApi');?>",
					data:data,
					dataType:'json',
					success:function(data){
						if(data.code == 1){
							var url = "<?php echo U('Library/api');?>?libId="+libId;
	...
	<div class="choose">
		<div class="kind outer">
			...
			<div class="add_mach example12">
				<a href="#">
					<img src="/Public/Run/img/xzapi.png" />
				</a>
			</div>
		</div>
	</div>
	<div class="main_biaoge">
		<table  class="table1 table-bordered table-striped table table15">
			...
			<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
					<td><?php echo ($v["api_id"]); ?></td>
					<td><?php echo ($v["api_type_id"]); ?></td>
					<td><?php echo ($v["api_url"]); ?></td>
					<td><?php echo ($v["library"]); ?></td>
					<td class="outer">
						<a href="#" class="lib_bianji example13" attr-apiId="<?php echo ($v["api_id"]); ?>">编辑</a>
						<a href="javaScript:;" class="del" attr-id="<?php echo ($v["api_id"]); ?>" attr-libId="<?php echo ($v["library_id"]); ?>">删除</a>
					</td>
